...
{
    public class GetLeaveRequestAllStatusByIdCommandHandler : IRequestHandler<GetLeaveRequestAllStatusCommand, List<LeaveRequestDto>>
    {
        private readonly ILeaveRequestRepository _leaveRequestRepository;
        private readonly IMapper _mapper;

        public GetLeaveRequestAllStatusByIdCommandHandler(ILeaveRequestRepository leaveRequestRepository, IMapper mapper)
        {
            _leaveRequestRepository = leaveRequestRepository;
            _mapper = mapper;
        }

        public async Task<List<LeaveRequestDto>> Handle(GetLeaveRequestAllStatusCommand request, CancellationToken cancellationToken)
        {
            // Retrieve leave requests with the specified leave status and where status is not 1 (assuming status is an int field)
            var leaveRequests = await _leaveRequestRepository.GetAllByStatus(request.LeaveStatus);
