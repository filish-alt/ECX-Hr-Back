...
{
    public class GetReceiptFileByIdCommandHandler : IRequestHandler<GetReceiptFileByIdCommand, byte[]>
    {

        private readonly IMedicalFundRepository _medicalRepository;
        private readonly IMapper _mapper;

        public GetReceiptFileByIdCommandHandler(IMedicalFundRepository medicalRepository, IMapper Mapper)
        {
        ...
        public async Task<byte[]> Handle(GetReceiptFileByIdCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (fileEntity != null)
                {
                    string base64String = fileEntity.ReceiptFile;
